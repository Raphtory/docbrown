...
impl GraphViewInternals for TemporalGraph {
                ...
                if let Some(window) = &vertex.w {
                    match direction {
                        ...
                        Direction::BOTH => Box::new(
                            GraphViewInternals::neighbours(self, vertex, Direction::IN)
    ...
    fn property_history<'a>(
        ...
        match &vertex.w {
            Some(r) => Some(meta.iter_window(*prop_id, r.clone())),
            None => Some(meta.iter(*prop_id)),
        }
    }
}
...
mod graph_test {
    ...
    fn add_vertex_at_time_t1() {
        ...
        assert!(g.contains_vertex_window(9, 1..15));
        assert_eq!(
            g.iter_vertices().map(|v| v.id()).collect::<Vec<u64>>(),
            vec![9]
        );
    ...
    fn add_vertices_with_1_property() {
        ...
        assert!(g.contains_vertex_window(v_id, 1..15));
        assert_eq!(
            g.iter_vertices().map(|v| v.id()).collect::<Vec<u64>>(),
            vec![v_id]
        );
    ...
    fn add_vertices_with_multiple_properties() {
        ...
        let res = g
            .iter_vertices()
            .flat_map(|v| {
                let type_ = v.property_history("type").map(|x| x.collect::<Vec<_>>());
                let active = v.property_history("active").map(|x| x.collect::<Vec<_>>());
    ...
    fn add_vertices_with_1_property_different_times() {
        ...
        let res: Vec<(&i64, Prop)> = g
            ...
            .flat_map(|v| {
                let type_ = v
                    .with_window(2..3)
                    .property_history("type")
                    .map(|x| x.collect::<Vec<_>>());
                let active = v
                    .with_window(2..3)
                    .property_history("active")
                    .map(|x| x.collect::<Vec<_>>());
                type_.zip(active).map(|(mut x, mut y)| {
                    x.append(&mut y);
                    x
    ...
    fn add_vertices_with_multiple_properties_at_different_times_window() {
        ...
        let res = g
            ...
            .flat_map(|v| {
                ...
                let label = v
                    .with_window(2..5)
                    .property_history("label")
                    .map(|x| x.collect::<Vec<_>>());
                let origin = v
                    .with_window(2..5)
                    .property_history("origin")
                    .map(|x| x.collect::<Vec<_>>());
                type_
                    .zip(active)
                    .map(|(mut x, mut y)| {
    ...
    fn add_vertex_at_time_t1_window() {
        ...
        assert!(g.contains_vertex(9));
        assert!(g.contains_vertex_window(9, 1..15));
        assert!(g.contains_vertex_window(9, 5..15)); // FIXME: this is wrong and we might need a different kind of window here
    }
    ...
    fn add_vertex_at_time_t1_t2() {
        ...
        let actual: Vec<u64> = g.iter_vertices_window(0..2).id().collect();
        assert_eq!(actual, vec![9]);
        let actual: Vec<u64> = g.iter_vertices_window(2..10).map(|v| v.id()).collect();
        assert_eq!(actual, vec![1]);
        let actual: Vec<u64> = g.iter_vertices_window(0..10).map(|v| v.id()).collect();
    ...
    fn add_edge_at_time_t1() {
        ...
        let actual: Vec<u64> = g.iter_vertices_window(3..10).map(|v| v.id()).collect();
    ...
    fn add_edge_with_1_property_different_times() {
        ...
        let v11 = g.vertex(11).unwrap();
        let edge_weights = g
            .edges(&v11.with_window(4..8), Direction::OUT)
            .flat_map(|e| {
        ...
        let edge_weights = g
            .edges(&v22.with_window(4..8), Direction::IN)
            .flat_map(|e| {
                e.props_window("amount", 4..8).map(|i| {
                    i.flat_map(|(t, prop)| match prop {
                        Prop::U32(weight) => Some((t, weight)),
    ...
    fn add_edges_with_multiple_properties_at_different_times_window() {
        ...
        let v11_3_5 = g.vertex_window(11, 3..5).unwrap();
        let edge_weights = g
            .edges(&v11_3_5, Direction::OUT)
            .flat_map(|e| {
                let weight = e
    ...
    fn correctness_degree_test() {
        ...
        for v in g.iter_vertices() {
            let out1 = g
                ...
                .collect_vec();
            let out2 = g
                .edges(&v.with_window(1..7), Direction::OUT)
                .map(|e| e.global_dst())
                .collect_vec();
            assert_eq!(out1, out2);

            assert_eq!(
                g.degree(&v, Direction::OUT),
                g.degree(&v.with_window(1..7), Direction::OUT)
            );
            assert_eq!(
                g.degree(&v, Direction::IN),
                g.degree(&v.with_window(1..7), Direction::IN)
            );
        }

        let degrees = g
            .iter_vertices()
            .map(|v| (v.id(), v.in_degree(), v.out_degree(), v.degree()))
            .collect_vec();
