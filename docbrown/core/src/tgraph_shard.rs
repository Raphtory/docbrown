...
use crate::tgraph::{EdgeRef, PropertyError, TemporalGraph, VertexRef};
...
impl TGraphShard {
    ...
    pub fn add_vertex<T: InputVertex>(&self, t: i64, v: T, props: &Vec<(String, Prop)>) -> Result<(), PropertyError> {
        self.write_shard(move |tg| tg.add_vertex_with_props(t, v, props))
    }

    pub fn add_vertex_properties(&self, v: u64, data: &Vec<(String, Prop)>) -> Result<(), PropertyError> {
    ...
    pub fn add_edge_properties(&self, src: u64, dst: u64, data: &Vec<(String, Prop)>) -> Result<(), PropertyError> {
