...
impl Neo4JConnection {
    ...
    pub async fn load_query_into_graph(
        ...
        let mut result = self.neo_graph.execute(
            query
        ).await.unwrap();
...
mod neo_loader_test {
    ...
    fn load_movies(row:Row,graph:&raphtory::Graph) {
        ...
        graph.add_vertex(actor_born, actor_name.clone(), &vec![]).unwrap();
        graph.add_vertex_properties(actor_name.clone(), &vec![("type".into(), Prop::Str("actor".into()))]).unwrap();
        graph.add_vertex(film_release, film_title.clone(), &vec![]).unwrap();
        graph.add_vertex_properties(film_title.clone(), &vec![("type".into(), Prop::Str("film".into())), ("tagline".into(), Prop::Str(film_tagline))]).unwrap();
        graph.add_edge(film_release, actor_name, film_title, &vec![], Some(relation_type.as_str())).unwrap();
    }

    #[tokio::test]
    #[ignore = "Need to work out how to dummy neo for this"]
    async fn test_movie_db() {
        let neo = Neo4JConnection::new("127.0.0.1:7687".to_string(), "neo4j".to_string(), "password".to_string(), "neo4j".to_string()).await.unwrap();
        let doc_graph = raphtory::Graph::new(1);

        neo.load_query_into_graph(&doc_graph,query("MATCH (actor:Person) -[relation]-> (film:Movie) RETURN actor,relation,film"),load_movies).await.unwrap();
        println!("{}", doc_graph.latest_time().unwrap());
        println!("{}", doc_graph.earliest_time().unwrap());
    }
    ...
    async fn test_neo_connection() {
        ...
        neo.run(query("CREATE (p:Person {id: $id})").param("id", 3)).await.unwrap();

        let mut result = neo.execute(
            query("MATCH (p:Person {id: $id}) RETURN p").param("id", 3)
        ).await.unwrap();
