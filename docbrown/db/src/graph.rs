...
use docbrown_core::{
    tgraph::{EdgeRef, PropertyError, VertexRef},
...
impl Graph {
    ...
    pub fn add_vertex_properties<T: InputVertex>(&self, v: T, data: &Vec<(String, Prop)>) -> Result<(), PropertyError> {
        let shard_id = utils::get_shard_id_from_global_vid(v.id(), self.nr_shards);
    ...
    pub fn add_edge_properties<T: InputVertex>(&self, src: T, dst: T, props: &Vec<(String, Prop)>) -> Result<(), PropertyError> {
